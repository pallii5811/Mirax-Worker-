...
def _is_mobile(phone: str) -> bool:
    d = _normalize_phone_digits(phone)
    return bool(d) and d.startswith("3")
...
<body>
  <div class=\"wrap\">
    <div class=\"brand\">
      <div class=\"pill\">GREEN ENERGY MODE</div>
      <h1>SOLAR SNIPER</h1>
      <div class=\"sub\">Energy Lead Extractor • Solo aziende con telefono • Cellulari in cima</div>
    </div>
    ...
    <div class=\"panel\">
      ...
      <div class=\"presets\">
        <button class=\"preset\" data-value=\"Fabbrica, Produzione, Industria\">🏭 CERCA FABBRICHE</button>
        <button class=\"preset\" data-value=\"Hotel, Albergo, Resort\">🏨 CERCA ALBERGHI</button>
        <button class=\"preset\" data-value=\"Ristorante, Pizzeria\">🍽️ CERCA RISTORAZIONE</button>
      </div>
      ...
      <div class=\"actions\" id=\"download\" style=\"display:none\">
        <a id=\"xlsx\" href=\"#\" target=\"_blank\" rel=\"noreferrer\">Scarica Excel pulito (.xlsx)</a>
      </div>
...
<script>
  ...
  const render = (rows) => {
    if (!rows || !rows.length) {
      $('results').innerHTML = "<div style='margin-top:16px;color:rgba(230,242,239,0.65)'>Nessun lead (o nessun telefono trovato).</div>";
      return;
    }
